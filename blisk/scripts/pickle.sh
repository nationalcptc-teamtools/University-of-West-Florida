...
# Minimal update for fresh install
sudo apt update

echo "1. Installing 'uv' package manager and updating PATH."
# Curl for uv installer
curl -LsSf https://astral.sh/uv/install.sh | sh > /dev/null 2>&1
export PATH="$HOME/.local/bin:$PATH"
echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.zshrc

echo "2. Installing Python-based Active Directory tools via uv (Latest Git Versions)."
# Only install the latest versions of Python tools via uv
...
echo "3. Running BloodHound configuration script."
# This is the crucial step to initialize the Neo4j/PostgreSQL services
sudo bloodhound-setup
echo "   -> BloodHound services initialized and running on http://localhost:7474."
...
echo "4. Decompressing the rockyou wordlist (Kali default location)."
# Use the included rockyou.txt and decompress it
gunzip -k /usr/share/wordlists/rockyou.txt.gz 2>/dev/null
echo "   -> rockyou.txt copied and decompressed to /usr/share/wordlists/."
source ~/.zshrc
